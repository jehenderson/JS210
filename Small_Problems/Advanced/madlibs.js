...
// and adverbs directly into your program. Your program should read this text and,
...
// your template and this problem becomes much easier. Consequently, this exercise
// is a bit more open-ended since the input is also something that you'll be
// defining.
...
function madlibs(template) {
  ...
  return template.split(/\b/g).map(word => {
    return randomized[`${word}s`]
    ? randomized[`${word}s`][Math.floor(Math.random()*randomized[`${word}s`].length)]
    : word;
  }).join('');
}
